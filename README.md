...
| 🌍 **IP 属地** | 评论区「时间 \| 位置 \| 点赞」行显示发评论者的地理位置（uapis.cn 主查 + ip-api 兜底的 IP 属地，可叠加浏览器精确定位） |
...
| 配置项 | 默认 | 说明 |
|--------|------|------|
| 启用浏览器精确定位 | 关闭 | 评论者点击「提交评论」并通过输入校验后，浏览器请求定位授权；授权成功则用经纬度经高德反地理编码解析完整地址 |
| 高德 Web 服务 Key | 空 | 高德开放平台申请的 Web 服务 Key，**仅服务端调用，不暴露在前端** |

> 流程：前端 `navigator.geolocation` 拿经纬度（`enableHighAccuracy` 高精度 + **精度过滤**：误差 > 2000m 的 IP 级粗定位丢弃降级）→ 塞入 hidden 字段随评论提交 → 服务端调 `restapi.amap.com/v3/geocode/regeo` 反地理编码 → 完整地址按评论 ID（coid）持久化（`cache/comment_locations.json`，上限 5000 条，并发读写安全）。
> 显示优先级：**该评论的精确地址 > IP 属地**（无精确地址才回退 uapis / ip-api）。
> 评论者拒绝授权 / 定位超时（15s）/ 精度过滤未通过 / 非 HTTPS / 不支持定位时，静默降级为 IP 属地，不影响评论提交。
> 注：桌面 Chrome 国内因 Google 定位服务不可用，可能返回 IP 级粗定位（如定位到省会），会被精度过滤丢弃；手机浏览器走系统 GPS，精度正常。
